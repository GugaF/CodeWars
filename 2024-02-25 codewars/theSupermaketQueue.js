...
function queueTime(customers, n) {
    ...
    // create n cashiers, with 0 time values
    for(let i = 0; i < n; i++)
      arr[i] = 0;
    
    for(let j = 0; j < customers.length; j++) {
      // add customer's duration to current cashier's time taken
      arr[0] += customers[j];      
      // sort arr to have next available cashier at index 0
      arr.sort((a,b) => a - b)
    }
    
    // return cahier with the most time taken
...
//         let nextCashier = arr.indexOf(Math.min(...arr))
//         arr[nextCashier] += customer;
//     });
//     return Math.max(...arr);
// }
